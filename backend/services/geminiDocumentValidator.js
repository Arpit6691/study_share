...
const extractPdfText = async (filePath) => {
  try {
    ...
    throw new Error('Failed to parse PDF document. The file may be corrupted or invalid.');
  }
};
...
const validateStudyDocument = async (text, metadata = {}) => {
  ...
  if (!apiKey) {
    ...
    throw new Error('Document validation is temporarily unavailable. Please try again.');
  }
  ...
  try {
    ...
    console.log('[DocumentValidation] Gemini classification completed');
    ...
    try {
      ...
      // Attempt cleanup if markdown ticks are present
    ...
    console.error('[DocumentValidation] Gemini validation error:', error.message);
    throw new Error('Document validation is temporarily unavailable. Please try again.');
  }
};
